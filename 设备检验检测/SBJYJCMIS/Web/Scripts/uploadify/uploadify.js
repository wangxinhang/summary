...
(function ($) {
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '选择的文件无法加入上传队列:';
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('选择上传的文件 "' + file.name + '" 已经在上传队列肿了。\n您想将队列中的文件替换为当前选择的文件吗？');
                    if (!replaceQueueItem) {
            ...
            var thSrc = "../Image/image.png";
            if (file.type == ".docx" || file.type == ".doc") {
                thSrc = "../Image/Word.png";
            }
            if (file.type == ".xls" || file.type == ".xlsx") {
                thSrc = "../Image/Excel.png";
            }
            if (file.type == ".pdf") {
                thSrc = "../Image/Excel.png";
            }
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\n选择上传的文件数量已经超出系统限制的 ' + (parseInt(errorMsg) + 1) + ' 个文件。';
                        } else {
                            this.queueData.errorMsg += '\n选择上传的文件其大小之和超出系统限制的 (' + settings.queueSizeLimit + ')大小。';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n选择上传的文件 "' + file.name + '" 超出系统限制的(' + settings.fileSizeLimit + ')大小.';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n选择上传的文件 "' + file.name + '" 为空。';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n选择上传的文件 "' + file.name + '"  类型不正确(' + settings.fileTypeDesc + ')。';
                        break;
                }
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            switch (errorCode) {
                ...
                case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                    alert('已经达到上传大小限制 (' + errorMsg + ')。');
                    errorString = '超出上传大小限制';
                    break;
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('服务器上已经存在文件 "' + file.name + '" 。\n 要替换它吗？');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
    ...
    $.fn.uploadify = function (method) {
        ...
        if (methods[method]) {
            ...
            $.error('方法 ' + method + ' 不存在于$.uploadify');
        }

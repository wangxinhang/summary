...
//#region 对象SzdwTree
function SzdwTree(position, x, y, width, height) {
    //#region 一般属性
    ...
    this.maxHeight = 600; //对象的最大高度：默认400；
    ...
    this.expandorPosition = 0; //0:左边；1：右边.默认：0
    this.expandorShowMode =1; //0:项悬停显示；1：始终显示.默认：0
    this.hasCheckBox = 0; //项是否有选择框，0或false:无；1或true：有.默认：0或false    
    this.hasSearchBox = 1; //是否有搜索框，0或false:无；1或true：有.默认：0或false
    ...
    //#region 主题样式
    this.themeType = 0; //0：浅灰色；1：黑色，默认：1
    this.themeJsonList = [
    {
        loadingImgUrl: "/SzdwControls/image/loading/loading16b.gif",
        panel: { "color": "black", "background-color": "White", "border-color": "#D0D0D0" },//#E6E6E6
        searchPanel: { "color": "black", "background-color": "transparent" },
        searchBox: { "color": "#AAAAAA", "background-color": "transparent", "border-color": "#E0E0E0", "clearButtonImage": "/SzdwControls/image/tree/1/cancel21b.png" },
        searchResultPanel: { "color": "black", "background-color": "#F6F6F6", "border-color": "#E0E0E0" },
        listPanel: { "color": "black", "background-color": "transparent" },
        item: { "color": "black" },
        itemHover: { "color": "black", "background-color": "#DDDDDD" },
        itemSelected: { "color": "black", "background-color": "#EFCF5F" },//, "border-left": "3px solid  #EFCF5F"
        itemExpandor: { "color": "black", "background-color": "transparent", "itemExpandorImage": "/SzdwControls/image/tree/1/right20b4.png", "itemExpandorImageToggle": "/SzdwControls/image/tree/1/down20b4.png" },
        itemExpandorHover: { "color": "black", "background-color": "transparent" },//"background-color": "#E9E9E9"
        insertPanel: { "color": "black", "background-color": "transparent" },
        insertButton: { "color": "black", "background-color": "transparent", "insertButtonImage": "/SzdwControls/image/tree/1/insert21b.png" },
        insertButtonHover : { "background-color":"#F6F6F6" }
    },
    {
        loadingImgUrl: "/SzdwControls/image/loading16w.gif",
        panel: { "color": "white", "background-color": "#060606", "border-color": "#060606" },
        searchPanel: { "color": "white", "background-color": "transparent" },
        searchBox: { "color": "white", "background-color": "#333333", "border-color": "#333333", "clearButtonImage": "../SzdwControls/image/tree/2/cancel21w.png" },
        ...
        itemSelected: { "color": "white", "background-color": "#333333" },
        itemExpandor: { "color": "white", "background-color": "transparent", "itemExpandorImage": "/SzdwControls/image/tree/2/right20w.png", "itemExpandorImageToggle": "/SzdwControls/image/tree/2/down20w.png" },
        itemExpandorHover: { "color": "White", "background-color": "#505050" },
        insertPanel: { "color": "White", "background-color": "transparent" },
        insertButton: { "color": "White", "background-color": "transparent", "insertButtonImage": "/SzdwControls/image/tree/2/insert21w.png" },
    ...
    this.searchImage = "/SzdwControls/image/tree/search20ds.png";
    this.searchBox = null;
    ...
    this.clearButtonImage = "/SzdwControls/image/tree/cancel21b.png";
    //#endregion 搜索面板
    ...
    this.listPanelCssJson = { "overflow": "hidden", "border": "0px solid red", "background-color": "transparent" };
    this.listPanelScrollTop = 0;
    this.listBox = null;
    //#endregion 列表面板

    //#region 选择框
    this.checkBoxSize = 13;//选择框尺寸
    this.checkBoxList = [];//选择框对象数组
    //#endregion 选择框

    //#region 项Item
    this.itemList = [];//item对象数组
    this.itemWidth = 0;// width;
    this.itemHeight = 32;//24;
    ...
    this.itemExpandorImage = "/SzdwControls/image/tree/right20w.png";
    this.itemExpandorImageToggle =  "/SzdwControls/image/tree/down20w.png";
    ...
    this.fnItemDbClick = null; //项双击时调用的外部方法
    this.parentItemShowChildrenType = 2;//父级属性的项显示子项的方式：1：单击展开，2：双击展开，默认：2
    this.parentItemClickCallBackType = 1;//父级属性的项触发外部事件的方式：1：单击触发，2：双击触发，默认：1
    ...
    this.insertButtonImage = "/SzdwControls/image/tree/insert21b.png";
...
//#endregion 对象SzdwTree

//#region 显示隐藏

//mode:0:淡入淡出，默认；1：滑动
SzdwTree.prototype.ShowToggle = function (mode, e) {
    //关闭其他控件
    if (!this.visible) {
...
//SzdwTree显示
SzdwTree.prototype.Show = function () {
    //定位
    this.SetPosition();
...
//SzdwTree隐藏
SzdwTree.prototype.Hide = function () {
    var szdwCtrl = this;
...
//SzdwTree显示
SzdwTree.prototype.SlideDown = function () {
    //定位
    this.SetPosition();
...
//SzdwTree隐藏
SzdwTree.prototype.SlideUp = function () {
    var szdwCtrl = this;
...
//滚动到完全可视位置
SzdwTree.prototype.ScrollToView = function () {
    var p = this.panel;
    if (!$(p).is(":hidden")) {
...
//#region 数据绑定
SzdwTree.prototype.DataBind = function () {
    //加载本地数据或异步从数据库获取数据
...
//异步从数据库获服务器取数据
SzdwTree.prototype.AjaxData = function () {
    //异步从数据库获服务器取数据   
    this.loadingImg = this.CreateLoadingImg();
    var x = this.GetOffsetX();
    var y = this.GetOffsetY();
    var w = this.targetControl ? $(this.targetControl).outerWidth() : this.width;
    var h = this.height;
...
//ajax异步加载数据成功后执行的函数
SzdwTree.prototype.AjaxSuccess = function (jsonList) {
    if (this.jsonList == null) {
        this.GetJsonList(jsonList);
        //this.SetPanel();
        this.Show();
...
//设置
SzdwTree.prototype.SetPanel = function () {
    //载入父容器
...
//加载
SzdwTree.prototype.Load = function () {
    //创建
    this.CreateTree();
    this.GetWidthHeight();
...
//加至命名控件
SzdwTree.prototype.AppendToNamingContainer = function () {
    var p = this.panel;
    var nc = this.namingContainer;
...
//目标控件阻止事件冒泡
SzdwTree.prototype.TargetControlStopPropagation = function () {
...
//#region 更新
SzdwTree.prototype.Update = function () {
    $(this.panel).empty();
    this.selectedItem = null;
    this.DataBind();
}

SzdwTree.prototype.ClearData = function () {
    this.jsonList = null;
}
//#endregion 更新
...
//获取控件的数据
SzdwTree.prototype.GetJsonList = function (jsonList) {
    if (jsonList) {
...
//获取所有顶级父数据
SzdwTree.prototype.GetTopParentJsonList = function () {
    if (this.jsonList == null) {
        return false;
    } else {
        var jsonList = this.jsonList;
        ...
        //获取所有顶级父数据
        this.topParentJsonList = $.grep(jsonList, function (item, i) {
...
SzdwTree.prototype.GetChildJsonList = function (parentItemValue) {
...
//根据id获取所有的父级数据
SzdwTree.prototype.GetParentJsonList = function (id) {
    var jsonList = this.jsonList;
...
//获取搜索结果数据
SzdwTree.prototype.GetSearchJsonList = function (searchText) {
    if (this.jsonList == null) {
        return null;
    } else {
        var jsonList = this.jsonList;
...
//获取父Id
SzdwTree.prototype.GetParentId = function (id) {
    var jsonList = this.jsonList;
    var parentId = null;
    if (jsonList) {
...
SzdwTree.prototype.GetValueByText = function (text) {
    var jsonList = this.jsonList;
    var value = null;
    if (jsonList) {        
        var fId = this.fieldId;
...
SzdwTree.prototype.GetTextByValue = function (value) {
    var jsonList = this.jsonList;
    var text = null;
    if (jsonList) {
        var fId = this.fieldId;
...
//根据Value(fieldId的值)获取Text(fieldName的值）
SzdwTree.prototype.GetDataIndexByValue = function (value) {
    var jsonList = this.jsonList;
    var index = null;
    if (jsonList) {
        var fId = this.fieldId;
...
//#region 创建控件
SzdwTree.prototype.CreateTree = function () {
    var len = this.jsonList.length;
    //var jsonList = len <= 20 ? this.jsonList : this.topParentJsonList; //顶级父数据
    //var jsonList = this.jsonList;//this.topParentJsonList; //顶级父数据
...
//#region 搜索面板
//创建搜索面板
SzdwTree.prototype.CreateSearchPanel = function () {
...
//创建搜索框
SzdwTree.prototype.CreateSearchBox = function () {
    ...
    $(s).attr("type", "text");
    this.SetStyleByCssName(s, cssName);
    this.SetStyleByCssJson(s, cssJson);

    var img = this.searchImage;
    var backColor = cssJson["background-color"];
    var backImg = "url(" + img + ") no-repeat 3px " + backColor;
    $(s).css({ "position": "absolute", "background": backImg, "padding-left": "24px" });

    $(s).outerWidth(w, true).outerHeight(h).css({ "line-height": $(s).height()+ "px" }).val(defaultText);
    ...
    if (navigator.userAgent.indexOf("MSIE") != -1) {
        bindType = 'propertychange';
    }
    $(s).bind(bindType, function () {
        var text = $.trim($(this).val());
        szdwCtrl.ShowSearchResultPanel(text);       
    })
    return s;
...
//创建搜索框
SzdwTree.prototype.CreateClearButton = function () {
...
//创建搜索结果列表
SzdwTree.prototype.ShowSearchResultPanel = function (searchText) {
...
//创建搜索结果列表面板
SzdwTree.prototype.CreateSearchResultPanel = function (x, y) {
    var p = document.createElement("div");
...
//创建listPanel
SzdwTree.prototype.CreateListPanel = function () {
    var p = document.createElement("div");
...
SzdwTree.prototype.CreateItemList = function (jsonList, itemType) {
    if (jsonList == null) {
        return null;
    } else {
...
//设置listPanel尺寸
SzdwTree.prototype.ResizeListPanel = function () {
    var sh = 0, ih = 0;
    if (this.hasSearchBox) {
        sh = $(this.searchPanel).outerHeight(true);
    }

    if (this.hasInsertButton) {
        ih = $(this.insertPanel).outerHeight(true);
    }

    var pw = $(this.panel).width();
    var ph = $(this.panel).height();
    var w = this.listPanelWidth > 0 ? this.listPanelWidth :pw;
    var h = ph - sh - ih;

    $(this.listPanel).outerWidth(w).outerHeight(h);
}
//#endregion 创建列表
...
SzdwTree.prototype.CreateItem = function (itemJsonParams) {
    //#region 相关变量
    ...
    if (itemType == 1) {//&& len > 20
        //复选框
        var chk = null;
        var chkWidth = this.checkBoxSize;
        if (hasCheckBox) {
            chk = document.createElement("input");
            $(chk).attr("type", "checkbox").css({ "float": "left", "margin": "0px" }).outerWidth(chkWidth).outerHeight(height);
            this.checkBoxList.push(chk);
            $(item).append(chk).data("checkbox",chk);
        }
        ...
        if (expandorImage) {
            ...
            var visibility = this.expandorShowMode == 0 ? "hidden" : "visible";
            ...
            $(itemExpandor).outerWidth(ew).outerHeight(height).css({ "float": "left", "visibility": visibility});
            if (hasChild) {
    ...
    if (image) {
        ...
        $(item).append(itemImg).data("itemImg", itemImg);
    }
    ...
    $(item).mouseover(function () {
        ...
        if (itemType == 1) {
            ...
            if (itemExpandor) {
                if (szdwCtrl.expandorShowMode == 0) {
                    //$(itemExpandor).show();
                    $(itemExpandor).css({ "visibility": "visible" });
                }
            }
        }
        ...
        if (itemType == 1) {
            ...
            if (itemExpandor) {
                if (szdwCtrl.expandorShowMode == 0) {
                    //$(itemExpandor).hide();
                    $(itemExpandor).css({ "visibility": "hidden" });
                }
            }
        }
    }).click(function (e) {
        //恢复选中项未选中前样式
        var selectedItem = szdwCtrl.selectedItem;
        if (selectedItem) {
            ...
            szdwCtrl.SetStyleByCssName(selectedItem, oldCssName);
            szdwCtrl.SetStyleByCssJson(selectedItem, oldCssJson);
        }
        ...
        if (hasChild) {
            //父级属性的项单击展开收起子项
            if (szdwCtrl.parentItemShowChildrenType == 1) { $(itemExpandor).trigger("click"); }
            //允许父级属性的项单击时触发外部事件
            if (szdwCtrl.parentItemClickCallBackType==1) {
                if (fnClick) {
                    fnClick(dataIndex, szdwCtrl);
                }
            }
        } else {            
            if (fnClick) {
        ...
        //调用外部方法
        var fnDbClick = szdwCtrl.fnItemDbClick;
        if (hasChild) {
            //父级属性的项双击展开或收起子项
            if (szdwCtrl.parentItemShowChildrenType == 2) { $(itemExpandor).trigger("click"); }                        
        } 

        //允许父项双击调用外部事件
        var flag = fnDbClick && hasChild && szdwCtrl.parentItemClickCallBackType == 2;
        if (flag) {
            fnDbClick(dataIndex, szdwCtrl);
        }
        //$(itemExpandor).trigger("click");
        //szdwCtrl.SetStyleByCssName(itemExpandor, expCssName);
        //szdwCtrl.SetStyleByCssJson(itemExpandor, expCssJson);
    });
    //#endregion 项事件绑定
    ...
    if (itemExpandor && hasChild) {
        $(itemExpandor).mouseover(function () {
            ...
            var listPanelIsMaxHeight = parseInt($(szdwCtrl.listPanel).css("max-height")) == $(szdwCtrl.listPanel).outerHeight();
...
//设置列表高度
SzdwTree.prototype.SetListPanelHeight = function () {
    var lb = this.listBox;
    var lbh = $(lb).outerHeight(true);
    //var mh = this.maxHeight;
    var mh = this.maxHeight > 0 ? Math.min(this.maxHeight, this.height) : this.height;
...
//#region 设置默认选项和选中项
//设置默认选项
SzdwTree.prototype.SetDefaultItem = function () {
...
//设置选中项
SzdwTree.prototype.SetSelectedItem = function (item) {
    var lp = this.listPanel;
...
//#region 新增数据面板
//创建新增数据面板
SzdwTree.prototype.CreateInsertPanel = function () {
    var p = document.createElement("div");
...
//创建新增数据面板
SzdwTree.prototype.CreateInsertButton = function () {
...
//定位
SzdwTree.prototype.SetPosition = function () {  
    var x = this.targetControl ? this.GetOffsetX() : this.x;
    var y = this.targetControl ? this.GetOffsetY() : this.y;
    var p = this.position;
...
//定位
SzdwTree.prototype.RePosition = function (x, y) {
...
//获得偏移X（含滚动）
SzdwTree.prototype.GetOffsetX = function () {
    var nc = this.namingContainer;
...
//获得偏移Y（含滚动）
SzdwTree.prototype.GetOffsetY = function () {
    var nc = this.namingContainer;
...
//重设尺寸
SzdwTree.prototype.Resize = function (width,height) {
    var p = this.panel;
    if (width > 0) {
        $(p).innerWidth(width);
        this.width = width;
    ...
    if (height > 0) {
        $(p).innerHeight(height);
        this.height = height;
    }

    this.ResizeListPanel();
}

//获取载入数据后的尺寸
SzdwTree.prototype.GetWidthHeight = function () {
...
//设置样式
SzdwTree.prototype.SetPanelStyle = function () {
    var nc = this.namingContainer;
    var ncSl = $(nc).scrollLeft();
    var ncSt = $(nc).scrollTop();
    this.x = this.x + ncSl;
    ...
    if (t) {
        ...
        var obw = tbw - pbw;;
        minWidth= this.minWidth + obw;

    }

    if (this.minWidth > 0) $(p).css("min-width", minWidth);
...
//设置样式
SzdwTree.prototype.SetListPanelStyle = function () {
    var t = this.targetControl;
...
//根据CssName设置Item样式
SzdwTree.prototype.SetStyleByCssName = function (element, cssName) {
    if (cssName != null) { $(element).addClass(cssName); }
}

//根据CssJson设置Item样式
SzdwTree.prototype.SetStyleByCssJson = function (element, cssJson) {
    if (cssJson != null) { $(element).css(cssJson); }
}
//#endregion 设置样式

//#region 设置主题样式
SzdwTree.prototype.SetThemeStyle = function () {
...
//#region 设置数据加载动画
SzdwTree.prototype.CreateLoadingImg = function () {
    var e = document.createElement("div");
    var i = this.themeType;
...
SzdwTree.prototype.ShowLoadingImg = function (x, y, width, height) {  
...
SzdwTree.prototype.HideLoadingImg = function () {
...
function CreateTreeBackImage(targetControl, imgUrl) {
    var t = targetControl;
//    if ($(t).val()) { $(t).val(""); }
//    if ($(t).text()) { $(t).text(""); }
...
function CreateTreeIcon(targetControl, iconUrl) {
    var t = targetControl;
    if (t) {
        if ($(t).val()) { $(t).val(""); }
        if ($(t).text()) { $(t).text(""); }
...
$.fn.extend({
    Tree: function (paramJson) {
        var c = this;
        $(c).css("cursor", "pointer");

        //创建下拉框图标
        var iconUrl = "/SzdwControls/image/tree/triangle.png";
        //CreateTreeIcon(c, iconUrl);
        //CreateTreeBackImage(c, iconUrl);
        ...
        $(c).unbind("click").click(function (e) {
            if (!c.szdwCtrl) {
                ...
                //创建实例
                c.szdwCtrl = new SzdwTree(position, x, y, w, h);

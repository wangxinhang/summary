...
//#region 对象SzdwAutoComplete
function SzdwAutoComplete(position, x, y, width, height) {
    //#region 一般属性
    ...
    //#endregion 对象SzdwAutoComplete

//#region 显示隐藏

//mode:0:淡入淡出，默认；1：滑动
SzdwAutoComplete.prototype.ShowToggle = function (mode, e) {
    //关闭其他控件
    if (!this.visible) {
...
SzdwAutoComplete.prototype.ShowToggle0 = function (mode, e) {
    //关闭其他控件
    if (!this.visible) {
        $(document).trigger("click");
    }
...
//SzdwAutoComplete显示
SzdwAutoComplete.prototype.Show = function () {
    //定位
    this.SetPosition();
...
//SzdwAutoComplete隐藏
SzdwAutoComplete.prototype.Hide = function () {
    var szdwCtrl = this;
...
//SzdwAutoComplete显示
SzdwAutoComplete.prototype.SlideDown = function () {
    //定位
    this.SetPosition();
...
//SzdwAutoComplete隐藏
SzdwAutoComplete.prototype.SlideUp = function () {
    var szdwCtrl = this;
...
//滚动到完全可视位置
SzdwAutoComplete.prototype.ScrollToView = function () {
    var p = this.panel;
    if (!$(p).is(":hidden")) {
...
//#region 数据绑定
SzdwAutoComplete.prototype.DataBind = function () {
    //加载本地数据或异步从数据库获取数据
    this.SetPanel();
    if (this.isReadonly) {
...
//异步从数据库获服务器取数据
SzdwAutoComplete.prototype.AjaxData = function () {
    //异步从数据库获服务器取数据   
...
//ajax异步加载数据成功后执行的函数
SzdwAutoComplete.prototype.AjaxSuccess = function (jsonList) {
    if (this.jsonList == null) {
        this.GetJsonList(jsonList);
        this.Load();
...
//设置
SzdwAutoComplete.prototype.SetPanel = function () {
    //载入父容器
...
//加载
SzdwAutoComplete.prototype.Load = function () {
    //创建   
    //if (event) {
...
//加至命名控件
SzdwAutoComplete.prototype.AppendToNamingContainer = function () {
    var p = this.panel;
    var nc = this.namingContainer;
...
//目标控件阻止事件冒泡
SzdwAutoComplete.prototype.TargetControlStopPropagation = function () {
...
//获取控件的数据
SzdwAutoComplete.prototype.GetJsonList = function (jsonList) {
    if (jsonList) {
...
//获取所有顶级父数据
SzdwAutoComplete.prototype.GetTopParentJsonList = function () {
    if (this.jsonList == null) {
        return null;
    } else {
        var jsonList = this.jsonList;
...
SzdwAutoComplete.prototype.GetChildJsonList = function (parentItemValue) {
...
//根据id获取所有的父级数据
SzdwAutoComplete.prototype.GetParentJsonList = function (id) {
    var jsonList = this.jsonList;
...
//获取搜索结果数据
SzdwAutoComplete.prototype.GetSearchJsonList = function (searchText) {
    if (this.jsonList == null) {
        return null;
    } else {
        var jsonList = this.jsonList;
...
//获取父Id
SzdwAutoComplete.prototype.GetParentId = function (id) {
    var jsonList = this.jsonList;
    var parentId = null;
    if (jsonList) {
...
SzdwAutoComplete.prototype.GetValueByText = function (text) {
    var jsonList = this.jsonList;
    var value = null;
    if (jsonList) {        
        var fId = this.fieldId;
...
SzdwAutoComplete.prototype.GetTextByValue = function (value) {
    var jsonList = this.jsonList;
    var text = null;
    if (jsonList) {
        var fId = this.fieldId;
...
//根据Value(fieldId的值)获取Text(fieldName的值）
SzdwAutoComplete.prototype.GetDataIndexByValue = function (value) {
    var jsonList = this.jsonList;
    var index = null;
    if (jsonList) {
        var fId = this.fieldId;
...
//#region 创建控件
SzdwAutoComplete.prototype.CreateDropDownList = function () {
...
//创建搜索面板
SzdwAutoComplete.prototype.CreateSearchPanel = function () {
...
//创建搜索框
SzdwAutoComplete.prototype.CreateSearchBox = function () {
...
//创建搜索框
SzdwAutoComplete.prototype.CreateClearButton = function () {
...
//创建搜索结果列表
SzdwAutoComplete.prototype.ShowSearchResultPanel = function (searchText) {
...
//创建搜索结果列表面板
SzdwAutoComplete.prototype.CreateSearchResultPanel = function (x, y) {
    var p = document.createElement("div");
...
//创建listPanel
SzdwAutoComplete.prototype.CreateListPanel = function () {
    var p = document.createElement("div");
...
SzdwAutoComplete.prototype.CreateItemList = function (jsonList, itemType) {
    if (jsonList == null) {
        return null;
    } else {
...
SzdwAutoComplete.prototype.CreateItem = function (itemJsonParams) {
    //#region 相关变量
...
//恢复选中项未选中前样式
SzdwAutoComplete.prototype.RecoverSelectedItem = function () {
...
//设置列表高度
SzdwAutoComplete.prototype.SetListPanelHeight = function () {
    var lb = this.listBox;
...
//设置默认选项
SzdwAutoComplete.prototype.SetDefaultItem = function () {
...
//设置选中项
SzdwAutoComplete.prototype.SetSelectedItem = function (item) {
    var lp = this.listPanel;
...
//#region 新增数据面板
//创建新增数据面板
SzdwAutoComplete.prototype.CreateInsertPanel = function () {
    var p = document.createElement("div");
...
//创建新增数据面板
SzdwAutoComplete.prototype.CreateInsertButton = function () {
...
//定位
SzdwAutoComplete.prototype.SetPosition = function () {  
    var x = this.GetOffsetX();
    var y = this.GetOffsetY();
    var p = this.position;
...
//获得偏移X（含滚动）
SzdwAutoComplete.prototype.GetOffsetX = function () {
    var nc = this.namingContainer;
...
//获得偏移Y（含滚动）
SzdwAutoComplete.prototype.GetOffsetY = function () {
    var nc = this.namingContainer;
...
//重设尺寸
SzdwAutoComplete.prototype.Resize = function () {
    var p = this.panel;
    var w = this.width;
    var h = this.height;
...
//获取载入数据后的尺寸
SzdwAutoComplete.prototype.GetWidthHeight = function () {
...
//设置样式
SzdwAutoComplete.prototype.SetPanelStyle = function () {
    var nc = this.namingContainer;
    var ncSl = $(nc).scrollLeft();
    var ncSt = $(nc).scrollTop();
    this.x = this.x + ncSl;
...
//设置样式
SzdwAutoComplete.prototype.SetListPanelStyle = function () {
    var t = this.targetControl;
...
//根据CssName设置Item样式
SzdwAutoComplete.prototype.SetStyleByCssName = function (element, cssName) {
    if (cssName != null) { $(element).addClass(cssName); }
}

//根据CssJson设置Item样式
SzdwAutoComplete.prototype.SetStyleByCssJson = function (element, cssJson) {
    if (cssJson != null) { $(element).css(cssJson); }
}
//#endregion 设置样式

//#region 设置主题样式
SzdwAutoComplete.prototype.SetThemeStyle = function () {
...
//#region 设置数据加载动画
SzdwAutoComplete.prototype.CreateLoadingImg = function () {
    var e = document.createElement("div");
    var i = this.themeType;
...
SzdwAutoComplete.prototype.ShowLoadingImg = function (x, y, width, height) {  
...
SzdwAutoComplete.prototype.HideLoadingImg = function () {
...
$.fn.extend({
    autoComplete: function (paramJson) {
        var c = this;
        $(c).css("cursor", "pointer");
        if (paramJson.isReadonly === undefined) {
            paramJson.isReadonly = true;
        }
        $(c).prop("readonly", !!paramJson.isReadonly);
        //创建下拉框图标
        ...
        $(c).unbind("click")
            .click(function (e) {
                if (!c.szdwCtrl) {
                    ...
                    //创建实例
                    c.szdwCtrl = new SzdwAutoComplete(position, x, y, w, h);
                    c.szdwCtrl.targetControl = this;
                    var param = {};
                    //var param = { inputText: $(this).val() };
                    //#region 传入参数
                    if (paramJson) {
                        c.szdwCtrl.isReadonly = paramJson.isReadonly;
                        if (paramJson.requestUrl) {
                            c.szdwCtrl.requestUrl = paramJson.requestUrl;
                        }

                        if (paramJson.paramJson) {
                            c.szdwCtrl.paramJson = $.extend(true, {}, param, paramJson.paramJson);
                        }

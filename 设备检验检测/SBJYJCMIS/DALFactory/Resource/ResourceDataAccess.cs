...
namespace SBJYJCMIS.DALFactory
{
    ...
    /// </summary>
    public sealed class ResourceDataAccess
    {
        private static readonly string path = ConfigurationManager.AppSettings["WebDAL"];

        // Look up the DAL implementation we should be using
        private ResourceDataAccess() { } 

        //获取用户信息
        public static IResource CreateResource()
        {
            string className = path + ".Resource";
            return (IResource)Assembly.Load(path).CreateInstance(className);
        }
    }
}

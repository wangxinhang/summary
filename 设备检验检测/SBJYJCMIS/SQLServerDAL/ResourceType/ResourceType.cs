...
namespace SBJYJCMIS.SQLServerDAL
{
    public class ResourceType:IResourceType
    {
        ...
        public bool ResourceTypeInsert(ResourceTypeInfo ResourceType)
        {
            ...
            parm[0].Value = ResourceType.Name;
            parm[1].Value = ResourceType.Memo;
            parm[2].Value = ResourceType.CreatorId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proResourceTypeInsert", parm);
            return (retval > 0) ? true : false;
        }

        //根据ResourceTypeId删除记录
        public bool ResourceTypeLogicDeleteById(int id, int modifierId)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proResourceTypeLogicDeleteById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        public bool ResourceTypeUpdateById(ResourceTypeInfo ResourceType)
        {
            ...
            parm[0].Value = ResourceType.Id;
            parm[1].Value = ResourceType.Name;
            parm[2].Value = ResourceType.DataStatusId;
            parm[3].Value = ResourceType.Memo;
            parm[4].Value = ResourceType.ModifierId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proResourceTypeUpdateById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        public bool ResourceTypeIsExistedByName(string name)
        {
            ...
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proResourceTypeIsExistedByName", parm);
            return obj == null ? false : (bool)obj;
        }
        #endregion 按条件读取记录结束

        #region 获取ResourceTypeList
        ...
        public IList<ResourceTypeListInfo> ResourceTypeGetList(string name, bool isPaged, int pageNumber, int pageSize, string orderFieldName, string orderType)
        {
            ...
            IList<ResourceTypeListInfo> recordList = new List<ResourceTypeListInfo>();
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proResourceTypeGetList", parm))
            {
                while (rdr.Read())
                {
                    ResourceTypeListInfo record = new ResourceTypeListInfo();
        ...
        //获取ResourceTypeListSum,记录总数和汇总值

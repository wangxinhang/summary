...
{
    public class FlowHandleType : IFlowHandleType
    {
        #region 参数
        //参数        
        ...
        //插入新记录
        public bool FlowHandleTypeInsert(FlowHandleTypeInfo flowRule)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowHandleTypeInsert", parm);
            return (retval > 0) ? true : false;
        }

        //更新记录
        public bool FlowHandleTypeUpdateById(FlowHandleTypeInfo flowRule)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowHandleTypeUpdateById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        //检验是否存在用户重名
        public bool FlowHandleTypeIsExistedByName(string name, int flowId)
        {
            ...
            parm[0].Value = name;
            parm[1].Value = flowId;
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowHandleTypeIsExistedByName", parm);
        ...
        public bool FlowHandleTypeIsExistedByNewNameAndOldName(string newName, string oldName)
        {
            ...
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowHandleTypeIsExistedByNewNameAndOldName", parm);
            return obj == null ? false : (bool)obj;
        }
        #endregion 按条件读取记录结束

        #region 获取动作结果列表根据动作id 流程ID

        //根据FlowOperationId获取FlowHandleType列表
        public IList<FlowHandleTypeListInfo> FlowHandleTypeGetListByFlowOperationId(int flowOperationId, int flowId)
        {
            SqlParameter[] parm = new SqlParameter[2];
            ...
            parm[0].Value = flowOperationId;
            parm[1].Value = flowId;

            //根据参数获取数据
            IList<FlowHandleTypeListInfo> recordList = new List<FlowHandleTypeListInfo>();

            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proFlowHandleTypeGetListByFlowOperationId", parm))
            {
                while (rdr.Read())
                {
                    FlowHandleTypeListInfo record = new FlowHandleTypeListInfo();

                    record.FlowHandleTypeId = Convert.ToInt32(rdr["FlowHandleTypeId"]);
                    record.FlowHandleType = rdr["FlowHandleType"].ToString();
                                       ...
                                       bool isPaged, int pageNumber, int pageSize, object orderFieldName, object orderType)
        {
            ...
            IList<FlowHandleTypeListInfo> FlowHandleTypeLists = new List<FlowHandleTypeListInfo>();
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proFlowHandleTypeGetList", parm))
            {
                while (rdr.Read())
                {
                    FlowHandleTypeListInfo FlowHandleTypeList = new FlowHandleTypeListInfo();
                    FlowHandleTypeList.RowNumber = Convert.ToInt32(rdr["RowNumber"]);
                    FlowHandleTypeList.Id = Convert.ToInt32(rdr["Id"]);
                    FlowHandleTypeList.Name = rdr["Name"].ToString();
                    FlowHandleTypeList.FlowId = Convert.ToInt32(rdr["FlowId"]);
                    FlowHandleTypeList.Flow = rdr["Flow"].ToString();
                    FlowHandleTypeList.Memo = rdr["Memo"].ToString();
                    FlowHandleTypeList.IsAssigned = Convert.ToBoolean(rdr["IsAssigned"]);
                    FlowHandleTypeLists.Add(FlowHandleTypeList);
                }
                rdr.Close();
            }
            return FlowHandleTypeLists;
        }

...
{
    public class FlowHandleResult : IFlowHandleResult
    {
        #region 参数
        //参数        
        ...
        //插入新记录
        public bool FlowHandleResultInsert(FlowHandleResultInfo flowRule)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowHandleResultInsert", parm);
            return (retval > 0) ? true : false;
        }

        //更新记录
        public bool FlowHandleResultUpdateById(FlowHandleResultInfo flowRule)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowHandleResultUpdateById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        //检验是否存在用户重名
        public bool FlowHandleResultIsExistedByName(string name, int flowId)
        {
            ...
            parm[0].Value = name;
            parm[1].Value = flowId;
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowHandleResultIsExistedByName", parm);
        ...
        public bool FlowHandleResultIsExistedByNewNameAndOldName(string newName, string oldName)
        {
            ...
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowHandleResultIsExistedByNewNameAndOldName", parm);
            return obj == null ? false : (bool)obj;
        }
        #endregion 按条件读取记录结束
        ...
        //根据FlowOperationId获取FlowHandleResult列表
        public IList<FlowHandleResultListInfo> FlowHandleResultGetListByFlowOperationId(int flowOperationId, int flowId)
        {
            SqlParameter[] parm = new SqlParameter[2];
            ...
            //根据参数获取数据
            IList<FlowHandleResultListInfo> recordList = new List<FlowHandleResultListInfo>();

            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proFlowHandleResultGetListByFlowOperationId", parm))
            {
                while (rdr.Read())
                {
                    FlowHandleResultListInfo record = new FlowHandleResultListInfo();

                    record.FlowHandleResultId = Convert.ToInt32(rdr["FlowHandleResultId"]);
                    record.FlowHandleResult = rdr["FlowHandleResult"].ToString();
                                       ...
                                       bool isPaged, int pageNumber, int pageSize, object orderFieldName, object orderType)
        {
            ...
            IList<FlowHandleResultListInfo> FlowHandleResultLists = new List<FlowHandleResultListInfo>();
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proFlowHandleResultGetList", parm))
            {
                while (rdr.Read())
                {
                    FlowHandleResultListInfo FlowHandleResultList = new FlowHandleResultListInfo();
                    FlowHandleResultList.RowNumber = Convert.ToInt32(rdr["RowNumber"]);
                    FlowHandleResultList.Id = Convert.ToInt32(rdr["Id"]);
                    FlowHandleResultList.Name = rdr["Name"].ToString();
                    ...
                    FlowHandleResultList.IsAssigned = Convert.ToBoolean(rdr["IsAssigned"]);

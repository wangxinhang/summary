...
namespace SBJYJCMIS.SQLServerDAL
{
    public class Flow:IFlow
    {
        ...
        public bool FlowInsert(FlowInfo flow)
        {
            ...
            parm[0].Value = flow.Name;
            parm[1].Value = flow.Memo;
            parm[2].Value = flow.CreatorId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowInsert", parm);
        ...
        public bool FlowUpdateById(FlowInfo flow)
        {
            ...
            parm[1].Value = flow.Name;
            parm[2].Value = flow.DataStatusId;
            parm[3].Value = flow.Memo;
            parm[4].Value = flow.ModifierId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proFlowUpdateById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        //检验是否存在用户重名
        public bool FlowIsExistedByName(string name)
        {
            SqlParameter parm = new SqlParameter(PARM_Name, SqlDbType.NVarChar);
            parm.Value = name;

            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowIsExistedByName", parm);
        ...
        public bool FlowIsExistedByNewNameAndOldName(string newName, string oldName)
        {
            ...
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proFlowIsExistedByNewNameAndOldName", parm);
            return obj == null ? false : (bool)obj;
        }
        #endregion 按条件读取记录结束
        ...
        public IList<FlowInfo> FlowGetList(string name, bool isPaged, int pageNumber, int pageSize, string orderFieldName, string orderType)
        {
            ...
            IList<FlowInfo> recordList = new List<FlowInfo>();
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proFlowGetList", parm))
            {
                while (rdr.Read())
                {

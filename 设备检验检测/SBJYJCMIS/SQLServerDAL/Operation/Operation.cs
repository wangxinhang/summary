...
namespace SBJYJCMIS.SQLServerDAL
{
    public class Operation:IOperation
    {
        ...
        //插入新记录
        public bool OperationInsert(OperationInfo Operation)
        {
            ...
            parm[0].Value = Operation.Name;
            parm[1].Value = Operation.Memo;
            parm[2].Value = Operation.CreatorId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proOperationInsert", parm);
        ...
        public bool OperationDeleteTranById(int id)
        {
            ...
            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proOperationDeleteTranById", parm);
            return (retval > 0) ? true : false;
        }

        //更新记录
        public bool OperationUpdateById(OperationInfo Operation)
        {
            ...
            parm[0].Value = Operation.Id;
            parm[1].Value = Operation.Name;
            parm[2].Value = Operation.DataStatusId;
            parm[3].Value = Operation.Memo;
            parm[4].Value = Operation.ModifierId;

            int retval = SqlHelper.ExecuteNonQuery(SqlHelper.ConnectionString, "proOperationUpdateById", parm);
            return (retval > 0) ? true : false;
        }
        ...
        public bool OperationIsExistedByName(string name)
        {
            ...
            object obj = SqlHelper.ExecuteScalar(SqlHelper.ConnectionString, "proOperationIsExistedByName", parm);
            return obj == null ? false : (bool)obj;
        }
        #endregion 按条件读取记录结束

        #region 获取OperationList
        ...
        public IList<OperationListInfo> OperationGetList(string name, bool isPaged, int pageNumber, int pageSize, string orderFieldName, string orderType)
        {
            ...
            IList<OperationListInfo> recordList = new List<OperationListInfo>();
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proOperationGetList", parm))
            {
                while (rdr.Read())
                {
                    OperationListInfo record = new OperationListInfo();
        ...
        public OperationSumInfo OperationGetListSum(string name)
        {
            ...
            OperationSumInfo record = null;
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.ConnectionString, CommandType.StoredProcedure, "proOperationGetList", parm))
            {
                while (rdr.Read())
                {
                    record = new OperationSumInfo();
